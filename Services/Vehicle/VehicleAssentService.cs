...
{
    public class VehicleAssentService : IVehicleAssentService
    {
        private readonly PetrolSDContext _context;

        public VehicleAssentService(PetrolSDContext context){
            _context = context;
        }

        public async Task<VechileAssentResponse> GetVehicleAssentByQR(Guid id)
        {
            var AssentDetails= await _context.MvehicleTripAssent
            .Where(assent => assent.Qrcode == id)
            .Select(assent=>new VechileAssentResponse{
                AssentId = assent.AssentId,
                VehicleId = assent.VehicleId,
        ...
        public async Task<List<AssentReportResponse>> GetVehicleAssents(int StationID, string startDate, string endDate, int FuelTypeParam, string FullName, int StatusID)
        {
        ...
        [Obsolete]
        public async Task<VechileAssentResponse> PostVehicleAssents(Guid QrCode, int StationID, string RoleName, string UserName, int TripAuthProviderID, int TripAuthID, int FromLocalityID, int ToLocalityID)
        {
            // TODO Call Proc To Create Vehicle Assent 
            var VehicleAssent = await _context.MvehicleTripAssent
            .SingleOrDefaultAsync(vdata => vdata.Qrcode == QrCode);
            if (VehicleAssent == null)
                return null;

            if (VehicleAssent.StationId != StationID)
                return null;
            if (VehicleAssent == null)
                return null;
            ...
            // TODO Stored Procedure Should be return QrCode For Created Assent 

            // var StationIDP = new SqlParameter("@StationID", StationID);
            // var QrCodeP = new SqlParameter("@QrCode", QrCode);
            // var RoleNameP = new SqlParameter("@RoleName", RoleName);
            // var FromLocalityIDP = new SqlParameter("@FromLocalityID", FromLocalityID);
            // var ToLocalityIDP = new SqlParameter("@ToLocalityID", ToLocalityID);
            ...
            // _ = await _context.Database.ExecuteSqlCommandAsync("EXEC @retVal = [dbo].[BusTruckAssent] @QrCodeP,@StationIDP,@UserName,@RoleName,@FromLocalityID,@ToLocalityID", reternP, QrCodeP, StationIDP, UserNameP, RoleNameP, FromLocalityIDP, ToLocalityIDP);
        ...
        public async Task<bool> ValidStationVehicleAssent(Guid assentID, string StationID,string UserName)
        {
           ...
           if(Assent.StationId != int.Parse(StationID))
            ...
            //  var  machineAssent= await _context.MprofileFacilityAssent.FirstOrDefaultAsync(m => m.AssentId == AssentID);
             ...
             Assent.StatusId=2;//IMachineAssentServices.StatuseComplated;
...
//             var reternP = new SqlParameter() { ParameterName = "@retVal", SqlDbType = SqlDbType.Int, Direction = ParameterDirection.Output, Value = -1 };
//             await _context.Database.ExecuteSqlCommandAsync("EXEC @retVal = [dbo].[AssentWithdraw] @AssentID,@UserStationID,@UserName", reternP, AssentIDP, StationIDP,UserName);
//             int res = (int)reternP.Value;
//             if (res == -1)
//             {
//                 throw new HttpResponseException()
